...
-- 4+ 회원 먼저 200404/ 1034523명
  ...
  SELECT  
    ...
    ,NVL(A.연속무실적개월수_CA_12개월 ,0)              AS  연속무실적개월수_CA_12개월                                           
    ,'0'                                                  AS  연속무실적개월수_신판_CASC                                                                   
    ,0                                                    AS  최대연속증가개월수_할부_6개월_CASC                                                                
    ,NVL(B.이용카드사수_CA ,0)                         AS  이용카드사수_CA                                                               
    ...
    ,'999999'                                             AS  최종CA정상대체월                                                                                      
    ,TO_DATE('06060606', 'YYYYMMDD')                                     AS  최종CA한도조회일                                                                           
    ,0                                                    AS  ARS인터넷_30만원이하CA이용여부_6개월                                                                    
    ,0                                                    AS  상담경험여부_3개월                                                                             
    ,TO_DATE('06060606', 'YYYYMMDD')                                     AS  한도ARS_최종상담일                                                                           
    ,TO_DATE('06060606', 'YYYYMMDD')                                     AS  CA한도외_최종상담일                                                                          
    ,SYSDATE                                               AS  CR적재일시                                                                           
    FROM  DM.월개인본인회원실적 A           
         ,DM.월복수카드이용실적 B          
  ...
  SELECT  
    ...
    FROM (
      ...
      ,SYSDATE                                               AS  CRM적재일시 
         ...
         --         ON A.고객번호 = B.고객번호    
             FROM DM.월개인본인회원실적 A
        LEFT OUTER JOIN DM.월복수카드이용실적 B
           ON A.고객번호 = B.고객번호
           AND A.기준년월 = B.기준년월                         
            WHERE A.기준년월 = EDW_기준년월
  AND A.정상회원여부 = 1
...
------------------------------------------------------------------------------------------------------
  ...
  /* [TODO] Oracle은 UPDATE-FROM 미지원. MERGE 또는 서브쿼리로 변환 필요:
...
*/ A,
              (
     SELECT 본인고객번호
            ,SUBSTR(MIN(TO_CHAR(한도시작일자, 'YYYYMMDD')||변경전한도금액),9)     AS 변경전한도
        ,SUBSTR(MAX(TO_CHAR(한도시작일자, 'YYYYMMDD')||변경후한도금액),9)     AS 변경후한도
       FROM  DW.회원한도변경이력                      
      WHERE  TO_CHAR(변경일시, 'YYYYMMDD') BETWEEN EDW_5개월전월||'01'  AND  EDW_말일
        AND 한도구분코드 = '13'          
           GROUP BY  본인고객번호
    ) B
...
*/ A,
     ...
     SELECT 본인고객번호
        ...
        ,SUBSTR(MAX(TO_CHAR(한도시작일자, 'YYYYMMDD')||변경후한도금액),9)     AS 변경후한도
       FROM  DW.회원한도변경이력                      
      WHERE  TO_CHAR(변경일시, 'YYYYMMDD')  BETWEEN EDW_2개월전월||'01'  AND  EDW_말일
        AND 한도구분코드 = '21'          
           GROUP BY  본인고객번호
    ) B
    WHERE A.고객번호 = B.본인고객번호 ;                    
...
------------------------------------------------------------------------------------------------------
  ...
  /* [TODO] Oracle은 UPDATE-FROM 미지원. MERGE 또는 서브쿼리로 변환 필요:
   UPDATE ACRM.월개인CA이용SCORE
     SET A.이용건수_CA2_CASC = NVL(B.이용건수_개인본인_CA,0),
         A.target여부_CASC = 
         CASE WHEN NVL(B.이용건수_개인본인_CA,0) + A.이용건수_CA3_CASC + A.이용건수_CA4_CASC > 0
         THEN 1 ELSE 0 END
...
*/ A LEFT OUTER JOIN 
      ...
      ON A.고객번호 = B.고객번호
     AND B.기준년월 = EDW_기준년월
   WHERE A.기준년월 = TO_CHAR(ADD_MONTHS(DATE(EDW_기준년월||'01'), -2), 'YYYYMM')
   ;
  ...
  /* [TODO] Oracle은 UPDATE-FROM 미지원. MERGE 또는 서브쿼리로 변환 필요:
   UPDATE ACRM.월개인CA이용SCORE
     SET A.이용건수_CA3_CASC = NVL(B.이용건수_개인본인_CA,0),
         A.target여부_CASC = 
         CASE WHEN NVL(B.이용건수_개인본인_CA,0) + A.이용건수_CA2_CASC + A.이용건수_CA4_CASC > 0
         THEN 1 ELSE 0 END
    FROM ACRM.월개인CA이용SCORE
*/ A LEFT OUTER JOIN 
         DM.월개인본인회원실적 B
      ON A.고객번호 = B.고객번호
     AND B.기준년월 = EDW_기준년월
   WHERE A.기준년월 = TO_CHAR(ADD_MONTHS(DATE(EDW_기준년월||'01'), -3), 'YYYYMM')
   ;
  ...
  /* [TODO] Oracle은 UPDATE-FROM 미지원. MERGE 또는 서브쿼리로 변환 필요:
   UPDATE ACRM.월개인CA이용SCORE
     SET A.이용건수_CA4_CASC = B.이용건수_개인본인_CA,
         A.target여부_CASC = 
         CASE WHEN NVL(B.이용건수_개인본인_CA,0) + A.이용건수_CA2_CASC + A.이용건수_CA3_CASC > 0
         THEN 1 ELSE 0 END
    FROM ACRM.월개인CA이용SCORE
*/ A LEFT OUTER JOIN 
         DM.월개인본인회원실적 B
      ON A.고객번호 = B.고객번호
     AND B.기준년월 = EDW_기준년월
   WHERE A.기준년월 = TO_CHAR(ADD_MONTHS(DATE(EDW_기준년월||'01'), -4), 'YYYYMM')
   ;
...
*/   A,
     ...
     WHERE A.고객번호 = B.본인회원고객번호
             AND B.기준일자 = (CASE WHEN EDW_당월말일 > '20050301' THEN DATE(EDW_당월말일) 
                               ELSE TO_DATE('2005-03-01', 'YYYYMMDD') END) ;
...
*/ A
         ,(
    SELECT 고객번호
           ,TO_CHAR(MAX(매출일자), 'YYYYMM')          AS 최종CA정상대체월
      FROM DW.현금서비스확정 
     WHERE 거래구분코드 = '2' 
       AND 취소전표여부 <> 1     
       AND 매출일자  <=  DATE(EDW_당월말일)  
...
*/ A
         ,(
    SELECT 고객번호
           ,TO_CHAR(MAX(매출일자), 'YYYYMM') AS 최종CA강제연체대체월
      FROM DW.현금서비스확정 
     WHERE 거래구분코드 IN ('3','4') 
       AND 취소전표여부 <> 1     
       AND 매출일자  <=  DATE(EDW_당월말일)         
...
*/ A
        ,(
          SELECT 고객번호
            ...
            FROM DW.상담
           WHERE TO_CHAR(접촉일자, 'YYYYMM') BETWEEN EDW_2개월전월  AND EDW_기준년월
            AND 고객상담접촉구분코드 = '01'
        GROUP BY 고객번호 
         )B 
   WHERE A.고객번호 = B.고객번호 ;   
...
------------------------------------------------------------------------------------------------------         
    EXTRACT_DAT := '/ACRM_HOME/DAT/CDC/ACSAC월개인CA이용SCORE'||'$1'||'.dat';      
    EXTRACT_SQL := '
...
--     1) 월개인CA이용SCORE 모델변수 sam 파일 내리기
------------------------------------------------------------------------------------------------------         
    EXTRACT_DAT := '/ACRM_HOME/DAT/CDC/ACSAC월개인CA이용SCORE02.dat';      
    EXTRACT_SQL := '
